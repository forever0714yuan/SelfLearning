    ...
    <div class="com-container">
        <div class="com-title" :style="fontSize">
            <div class="title">
                ...
                <span class="iconfont select-icon" @click="chiceType = !chiceType" :style="fontSize">&#xe6eb;</span>
            </div>
            <div class="select" v-show="chiceType" :style="marginLeft">
                <div class="select-item" v-for="(type) in typeData" :key="type.key" @click="handleSelect(type.key)">
                    {{ type.text }}
                </div>
            </div>
        </div>
...
const titleShow = computed(() => {
    return allData.value[choiceKey.value]?.title
})
...
const updataCharts = () => {
    ...
    legendData.value = allData.value[choiceKey.value].data?.map(item => item.name)
